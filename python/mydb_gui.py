...
def confirm_booking(window):
    ...
    mydb.execute("INSERT INTO transactions (agent,id,name,txn_type,qty,cost,txnid) \
        values (?, ?, ?, ?, ?, ?, ?)", (current_user, int(pid_var_bs.get()), pname_var_bs.get(), txn_type_gl, int(qty_var_bs.get()), int(cost_var_bs.get()), maxtxnid))
    mydb.commit()
...
def buysell(txn_type):  ## {
    ...
    inp_pid_bs      = tk.Entry(buysell_page, textvariable=pid_var_bs, validatecommand=check_pid_exist_bs, validate='focusout')
    inp_pname    = tk.Entry(buysell_page, textvariable=pname_var_bs   )
    inp_pprice   = tk.Entry(buysell_page, textvariable=pprice_var_bs  )
    inp_qty = tk.Entry(buysell_page, textvariable=qty_var_bs, validatecommand=check_qty, validate='focusout')
    ...
    #
    global btn_confirm
    btn_confirm = tk.Button(buysell_page, text='Confirm', width=10, state=tk.DISABLED, command=lambda:confirm_booking(buysell_page))
...
def add_product():
    ...
    inp_pid      = tk.Entry(add_prod_page, textvariable=pid_var, validatecommand=check_pid_exist, validate='focusout')
    ...
    #
    btn_ok     = tk.Button(add_prod_page, text='OK', width=10, command=partial(add_new_prod, pid_var, pname_var, pprice_var, pminsell_var, pavail_var, add_prod_page))
    btn_cancel = tk.Button(add_prod_page, text='Cancel' , width=10, command=add_prod_page.destroy)
    btn_cancel.grid(row=5, column=2, pady=5, sticky='e')
...
global btn_addprod
global tree
# place the widgets on the main window
...
for i in range (0,9):
    win.columnconfigure (i,weight=10)
